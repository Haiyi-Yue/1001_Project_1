...
t4, p4 = zip(*results)
proportion4 = sum(1 for p in p4 if p <= 0.0025) / df_male_rating.shape[1]
print(proportion4)
...
t6, p6 = zip(*results)
proportion6 = sum(1 for p in p6 if p <= 0.0025) / df_OnlyChildren_rating.shape[1]
print(proportion6)
...
ks = stats.ks_2samp(df_movies_col_mean['Home Alone (1990)'], df_movies_col_mean['Finding Nemo (2003)'])
print(ks)
...
#starwar
sw1 = df_movies_col_mean['Star Wars: Episode 1 - The Phantom Menace (1999)']
sw2 = df_movies_col_mean['Star Wars: Episode II - Attack of the Clones (2002)']
sw3 = df_movies_col_mean['Star Wars: Episode IV - A New Hope (1977)']
sw4 = df_movies_col_mean['Star Wars: Episode V - The Empire Strikes Back (1980)']
sw5 = df_movies_col_mean['Star Wars: Episode VI - The Return of the Jedi (1983)']
sw6 = df_movies_col_mean['Star Wars: Episode VII - The Force Awakens (2015)']

f1, p1 = stats.f_oneway(sw1, sw2, sw3, sw4, sw5, sw6)
print(f1, p1)

#harrypotter
hp1 = df_movies_col_mean['Harry Potter and the Chamber of Secrets (2002)']
hp2 = df_movies_col_mean['Harry Potter and the Deathly Hallows: Part 2 (2011)']
hp3 = df_movies_col_mean['Harry Potter and the Goblet of Fire (2005)']
hp4 = df_movies_col_mean['Harry Potter and the Sorcerer\'s Stone (2001)']

f2, p2 = stats.f_oneway(hp1, hp2, hp3, hp4)
print(f2, p2)

#matrix
m1 = df_movies_col_mean['The Matrix (1999)']
m2 = df_movies_col_mean['The Matrix Reloaded (2003)']
m3 = df_movies_col_mean['The Matrix Revolutions (2003)']

f3, p3 = stats.f_oneway(m1, m2, m3)
print(f3, p3)

#indianajones
ij1 = df_movies_col_mean['Indiana Jones and the Kingdom of the Crystal Skull (2008)']
ij2 = df_movies_col_mean['Indiana Jones and the Last Crusade (1989)']
ij3 = df_movies_col_mean['Indiana Jones and the Raiders of the Lost Ark (1981)']
ij4 = df_movies_col_mean['Indiana Jones and the Temple of Doom (1984)']

f4, p4 = stats.f_oneway(ij1, ij2, ij3, ij4)
print(f4, p4)

#jurassicpark
jp1 = df_movies_col_mean['Jurassic Park (1993)']
jp2 = df_movies_col_mean['Jurassic Park III (2001)']
jp3 = df_movies_col_mean['The Lost World: Jurassic Park (1997)']

f5, p5 = stats.f_oneway(jp1, jp2, jp3)
print(f5, p5)

#pirates of the carribbean
pc1 = df_movies_col_mean['Pirates of the Caribbean: At World\'s End (2007)']
pc2 = df_movies_col_mean['Pirates of the Caribbean: Dead Man\'s Chest (2006)']
pc3 = df_movies_col_mean['Pirates of the Caribbean: The Curse of the Black Pearl (2003)']

f6, p6 = stats.f_oneway(pc1, pc2, pc3)
print(f6, p6)

#toystory
ts1 = df_movies_col_mean['Toy Story (1995)']
ts2 = df_movies_col_mean['Toy Story 2 (1999)']
ts3 = df_movies_col_mean['Toy Story 3 (2010)']

f7, p7 = stats.f_oneway(ts1, ts2, ts3)
print(f7, p7)

#batman
bm1 = df_movies_col_mean['Batman & Robin (1997)']
bm2 = df_movies_col_mean['Batman (1989)']
bm3 = df_movies_col_mean['Batman: The Dark Knight (2008)']
